...
var staticGenOnce sync.Once
var staticGenRequests = make(chan struct{}, 1)
var staticOutputDirOverride string

func StartStaticGenerator() {
	staticGenOnce.Do(func() {
		go func() {
			for range staticGenRequests {
				GenerateStatic()
			}
		}()
	})
...
func RequestGenerateStatic() {
	if !config.Cfg.Static.Enable {
		return
	}
	select {
	case staticGenRequests <- struct{}{}:
	default:
	}
}
