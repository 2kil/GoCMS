	...
	$(function () {
		...
		if (plugins.swiper.length) {
			...
			for (let i = 0; i < plugins.swiper.length; i++) {
				...
				if( node.getAttribute('data-loop')) {
					generated.loop = node.getAttribute('data-loop') === 'true';
				}

				if ( node.getAttribute( 'data-autoplay' ) ) {
					if( node.getAttribute( 'data-autoplay') === 'false' ) {
						generated.autoplay = false;
					} else {
						generated.autoplay = {
							delay: Number( node.getAttribute( 'data-autoplay' ) ) || 3000,
				...
				if( node.getAttribute('data-slide-effect')) {
					generated.effect = (node.getAttribute('data-slide-effect') === 'fade' ) ? 'fade' : 'slide';
				}

				if( node.getAttribute('data-simulate-touch')) {
					generated.simulateTouch = node.getAttribute('data-simulate-touch') === 'true';
				}
